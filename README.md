...
> [!IMPORTANT]
> 本项目不内置书源，也不提供任何书籍内容。首次启动后可导入自己拥有合法使用权的书源、本地 TXT、EPUB 或 PDF 文件。
...
在「设置 → 数据 → 备份与恢复」中可以手动创建、上传和恢复备份。新版备份使用 ZIP 格式，本应用也会继续识别旧版 JSON 备份。

备份窗口支持配置 `WebDAV`（网盘协议）服务地址、账号、应用密码和子目录。保存配置并测试连接后，可按需手动上传、刷新远端列表、下载、恢复或删除备份；不会自动上传或恢复。远端备份放在配置子目录根目录，文件名以 `backup` 开头，与 `Legado`（阅读）备份目录兼容，手机端可直接识别。

“同步阅读进度”默认开启，可在同一窗口关闭。开启后，进入、退出阅读页以及阅读过程中会与 `Legado`（阅读）的 `bookProgress`（阅读进度目录）同步当前章节和位置。
...
- 书源搜索、正文加载和 RSS 订阅等网络请求会自动读取本机系统代理，未启用时直接连接。
