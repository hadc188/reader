    ...
    <div v-else-if="book && memory" class="ai-shell">
      ...
      <main class="ai-content">
        ...
        <section v-else-if="activeTab === 'map'" class="map-panel">
          ...
          <div class="map-frame">
            <img v-if="memory.map?.imageUrl" :src="resolveAssetUrl(memory.map.imageUrl)" alt="世界地图" />
...
async function resetMemory() {
  if (!book.value) return
  const ok = await appStore.confirmDialog('确定重置当前书的 AI资料？', { title: '重置AI资料', danger: true })
  if (!ok) return
