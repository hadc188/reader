// vendor 副本: 上游 0.12.0 的存量警告在此统一豁免, 保持对上游的最小 diff。
#![allow(unused)]
...
fn make_font<'a>(doc: &'a Document, font: &'a Dictionary) -> Rc<dyn PdfFont + 'a> {
    ...
    if subtype == "Type0" {
        ...
        Rc::new(PdfType3Font::new(doc, font)) as Rc<dyn PdfFont>
    } else {
        Rc::new(PdfSimpleFont::new(doc, font)) as Rc<dyn PdfFont>
    }
}
...
/// 中文双字节预定义 CMap (GBK / GB2312 / Big5 系)。这类 PDF 常见于 LaTeX CJK
/// 排版产物, 往往不带 ToUnicode 表, 上游遇到会直接 panic。近似处理:
/// 字节映射按恒等 (2 字节本地码位即 CID), Unicode 表用 encoding_rs 按
/// 对应编码逐码位解码生成。副作用: W 宽度数组以真实 CID 为键, 恒等映射后
/// 查不到而落到默认宽度, 只影响空格/换行推断, 不影响提取出的文本内容。
fn legacy_cjk_encoding(name: &str) -> Option<&'static encoding_rs::Encoding> {
    match name {
        "GB-EUC-H" | "GB-EUC-V" | "GBpc-EUC-H" | "GBpc-EUC-V"
        | "GBK-EUC-H" | "GBK-EUC-V" | "GBK2K-H" | "GBK2K-V"
        | "GBKp-EUC-H" | "GBKp-EUC-V" => Some(&encoding_rs::GBK),
        "B5pc-H" | "B5pc-V" | "ETen-B5-H" | "ETen-B5-V" | "ETenms-B5-H" | "ETenms-B5-V"
        | "HKscs-B5-H" | "HKscs-B5-V" => Some(&encoding_rs::BIG5),
        _ => None,
    }
}

/// 嵌入式 CMap stream 的降级识别: CMapName 命中 GB/Big5 系名称, 或
/// CIDSystemInfo 的 Ordering 为 Adobe-GB1/CNS1。这类 CMap 常以分段
/// begincidrange 等合法变体书写, adobe-cmap-parser 0.4 解析不了,
/// 但其源码位本身就是 GB/Big5 本地编码, 恒等近似即可正确解码。
fn stream_cmap_legacy_encoding(stream: &Stream) -> Option<&'static encoding_rs::Encoding> {
    if let Ok(Object::Name(name)) = stream.dict.get(b"CMapName") {
        if let Some(enc) = legacy_cjk_encoding(&pdf_to_utf8(name)) {
            return Some(enc);
        }
    }
    let ordering = stream
        ...
        .and_then(|d| d.get(b"Ordering").ok())
        .and_then(|o| match o {
        ...
        .unwrap_or_default();
    match ordering.trim() {
        "GB1" => Some(&encoding_rs::GBK),
        "CNS1" => Some(&encoding_rs::BIG5),
        _ => None,
    }
}

/// GBK 与 Big5 的双字节区间同为 lead 0x81..=0xFE, trail 0x40..=0xFE (跳过 0x7F),
/// 即 0x8140..=0xFEFE; 另带 ASCII 单字节段, 供 GB2312/Big5 系 CMap 的单字节码。
fn identity_dblbyte_mapping() -> ByteMapping {
    ByteMapping {
        codespace: vec![
            CodeRange { width: 1, start: 0x20, end: 0x7f },
            CodeRange { width: 2, start: 0x8140, end: 0xfefe },
        ],
        cid: vec![
            CIDRange { src_code_lo: 0x20, src_code_hi: 0x7f, dst_CID_lo: 0 },
            CIDRange { src_code_lo: 0x8140, src_code_hi: 0xfefe, dst_CID_lo: 0 },
        ],
    }
}

/// 按 encoding 逐双字节码位解码生成 Unicode 映射表 (约 2.4 万条, 进程内缓存)。
fn legacy_cjk_unicode_map(
    encoding: &'static encoding_rs::Encoding,
) -> &'static HashMap<u32, String> {
    static GBK_MAP: OnceLock<HashMap<u32, String>> = OnceLock::new();
    static BIG5_MAP: OnceLock<HashMap<u32, String>> = OnceLock::new();
    let lock = if encoding == encoding_rs::GBK { &GBK_MAP } else { &BIG5_MAP };
    lock.get_or_init(|| {
        let mut map = HashMap::new();
        // ASCII 单字节段 (恒等映射的 codespace 含 0x20..=0x7f)
        for b in 0x20u32..=0x7e {
            map.insert(b, char::from_u32(b).unwrap().to_string());
        }
...
impl<'a> PdfCIDFont<'a> {
    fn new(doc: &'a Document, font: &'a Dictionary) -> PdfCIDFont<'a> {
        ...
        let (encoding, legacy_cjk) = match encoding {
            &Object::Name(ref name) => {
                let name = pdf_to_utf8(name);
                dlog!("encoding {:?}", name);
                if name == "Identity-H" || name == "Identity-V" {
                    (ByteMapping { codespace: vec![CodeRange{width: 2, start: 0, end: 0xffff }], cid: vec![CIDRange{ src_code_lo: 0, src_code_hi: 0xffff, dst_CID_lo: 0 }]}, None)
                } else if let Some(enc) = legacy_cjk_encoding(&name) {
                    (identity_dblbyte_mapping(), Some(enc))
                } else {
                    panic!("unsupported encoding {}", name);
                }
            }
            &Object::Stream(ref stream) => {
                // 嵌入式 CMap: 名称/Registry 表明是 GB/Big5 系时直接用近似解码
                // (源码位即本地编码, 恒等映射 + 解码表), 绕过 adobe-cmap-parser
                // 对分段 begincidrange 等合法变体的解析失败。
                if let Some(enc) = stream_cmap_legacy_encoding(stream) {
                    (identity_dblbyte_mapping(), Some(enc))
                } else {
                    let contents = get_contents(stream);
                    dlog!("Stream: {}", String::from_utf8(contents.clone()).unwrap());
                    (adobe_cmap_parser::get_byte_mapping(&contents).unwrap(), None)
                }
            }
        ...
        if unicode_map.is_none() {
            if let Some(enc) = legacy_cjk {
                unicode_map = Some(legacy_cjk_unicode_map(enc).clone());
            }
        }
...
/// 字体缓存键。上游按资源名 (如 "F1") 缓存, 但资源名只在单个页面的
/// Resources 字典内唯一 —— 一个 Processor 跨多页复用时, 后续页的 "F1"
/// 会命中前一页的字体, 字形映射错位后 decode_char 返回空串, 表现为大段
/// 文字凭空消失。因此优先按字体对象 id 缓存, 只有内联字体字典 (无间接
/// 引用可用) 才退回按名字缓存 —— 该回退分支仍会跨页串字体, 是已知残余;
/// 实际 PDF 几乎都用间接引用, 未见触发。
...
struct Processor<'a> {
    font_table: HashMap<FontKey, Rc<dyn PdfFont + 'a>>,
    _none: PhantomData<&'a ()>,
}
        ...
        for operation in &content.operations {
            ...
            match operation.operator.as_ref() {
                ...
                "Tf" => {
                    ...
                    let name = operation.operands[0].as_name().unwrap();
                    let key = match fonts.get(name) {
                        Ok(&Object::Reference(id)) => FontKey::Object(id),
                        _ => FontKey::Name(name.to_owned()),
                    };
                    let font = self.font_table.entry(key).or_insert_with(|| make_font(doc, get::<&Dictionary>(doc, fonts, name))).clone();
                    {
...
/// 按显式页对象 id 列表逐页提取文本, 整段共享一个 `Processor`。
///
/// 相对上游的 `extract_text_by_pages`(每页新建 `Processor`, 字体与 CMap
/// 逐页重复解析, 千页中文书要一分钟以上), 这里字体只解析一次; 配合按
/// 对象 id 缓存的 `FontKey`, 跨页共享不会串font。
///
/// 取页对象 id 而不是页码, 便于调用方自行决定页序与范围。
pub fn extract_text_for_page_ids(
    doc: &Document,
    page_ids: &[ObjectId],
    ...
    for (offset, &object_id) in page_ids.iter().enumerate() {
